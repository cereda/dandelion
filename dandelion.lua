...
local function removeLineNumberReference(text)
    ...
    text = string.gsub(text, "%son%sline%s(%d+)", " on line ...")
    text = string.gsub(text, "^l%.(%d+)%s", "l... ")
    return text
end
...
local function printUsage()
    ...
    print("                                | --help | --version ]\n")
    print("<action>     four options are available:")
    print("               test : run tests on the provided file")
    print("               list : list available tests")
    print("               spec : save the test specs to a new file")
    print("               save : save the source to a reference file\n")
...
local function parseCommandLine(arguments)
    ...
    local actions = { "test" , "list", "spec", "save" }
...
-- This function generates the specification file from the content
...
--        and parsed.
local function generateSpec(ids, authors, groups, fromSource, fromLog, filename)
    ...
    -- '.spec' extension
    filename = getBasename(filename) .. ".spec"
    ...
    if fileHandler then
        ...
        fileHandler:write("% Specification file generated by dandelion on\n")
    ...
    -- print message
    print("The specification file '" .. filename .. "' was generated successfully.")
    
    -- something bad happened
    else
    
        -- print message
        print(":: I'm sorry, but the specification file could not be")
...
--- Gets the table reference, based on the provided ID.
-- This function gets the table reference based on the provided test ID.
-- @param mapping the table of mappings.
-- @param id The test ID.
-- @return A table containing all data from the provided test ID.
local function getTableReference(mapping, id)
...
-- This function generates the reference file based on the source code and
-- the log output.
-- @param filename The source filename.
-- @param fromSource The mapping table from the source code.
-- @param fromLog The mapping table from the log file.
local function generateReferenceFile(filename, fromSource, fromLog)

    -- let's perform the query and
    -- get the selected testes
    local selection = performQuery({}, {}, {}, fromSource, fromLog, true)
    ...
    -- create the reference filename
    local refFilename = getBasename(filename) .. ".tlg"
    
    -- create a file handler and open the
    -- file accordingly
    local fileHandler = io.open(filename, "r")
    local fileWriter = io.open(refFilename, "w")
    ...
    if fileHandler then
        ...
        if fileWriter then
            ...
            -- write a header with a date reference
            fileWriter:write("% Reference file generated by dandelion on\n")
            fileWriter:write("% " .. os.date("%A, %B %d, %Y, at %X.\n\n"))
            ...
            -- now let's analyze our bad boy
            for currentLine in fileHandler:lines() do
            
                -- if we enter the pattern
                if string.find(currentLine, "^%s*%%%s*!test$") then
                ...
                -- other lines  
                    ...
                    if grabber then
                        ...
                        if string.find(currentLine, "^%s*%%") then
                            ...
                            if string.find(currentLine,"^%s*%%%s*id%s*:(.*)$") then
                                
                                -- get the key
                                key = string.match(currentLine, '^%s*%%%s*id%s*:(.*)$')
                            
                                -- if it's not null, let's
                                -- trim it
                                if key ~= nil then
                            ...
                            -- we are in the output section
                            elseif string.find(currentLine, "^%s*%%%s*expects%s*:(.*)$") then
                            
                                -- set the line to zero
                                line = 0
                                
                            -- we are inside a codestring
                            elseif string.find(currentLine, "^%s*%%%%%%%s(.*)$") then
                            
                                -- increment counter
                                line = line + 1
                            
                            -- other comment lines, it seems
                            else
                ...
                -- we found a codestring,
                -- replace it by the correct
                -- reference
                elseif line == 1 then
                    ...
                    if key == nil then
                        ...
                        -- show message
                        print(":: I'm sorry, but I found a code block without having")
                        print(":: an ID yet defined. Please make sure to include the")
                        print(":: ID element before the code block. Stopping execution.")
                    ...
                    -- get the table reference according
                    -- to the provided test ID
                    local reference = getTableReference(selection, key)
                    
                    -- write the new result to the file
                    fileWriter:write("%%% " .. string.gsub(reference["output"], "\n", "\n%%%%%% ") .. "\n")
        ...
        -- writer is bad
            ...
            -- print message
            print(":: File '" .. refFilename .. "' does not exist or is unavailable.")
            print(":: Stopping execution.")
...
local function main()
    ...
    -- need the log output
        ...
        -- it's important to observe that the save
        -- action doesn't take command line arguments
        -- into consideration
        if action ~= "save" then
        
            -- a dirty trick: let's check for a valid
            -- query before running the TeX engine, so
            -- we save our time
        ...
        elseif action == "spec" then
            ...
            -- generate spec
            generateSpec(ids, authors, groups, fromSource, fromLog, filename)
        
        -- now we have the save part!
        else
        
            -- generate the reference file
            generateReferenceFile(filename, fromSource, fromLog)
